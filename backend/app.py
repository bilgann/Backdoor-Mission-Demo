...
# Demo read-only flag (default ON for portfolio/demo repos)
...
# Enforce read-only mode for demo portfolio deployments
@app.before_request
def enforce_demo_read_only():
    if DEMO_READ_ONLY and request.method in {"POST", "PUT", "PATCH", "DELETE"}:
        return jsonify({
            "message": "Demo is read-only. Writes are disabled.",
